...
balanced_data = pd.read_csv('balanced_sampled_with_gpt_sentiment.csv')

# Clean the 'gpt_sentiment' column to remove numerical prefixes or extra text
balanced_data['gpt_sentiment_cleaned'] = (
    balanced_data['gpt_sentiment']
...
# Filter out invalid or non-standard values in the cleaned 'gpt_sentiment' column
valid_gpt_sentiments = balanced_data['gpt_sentiment_cleaned'].isin(['positive', 'negative'])
filtered_data = balanced_data[valid_gpt_sentiments]

# Count missed sentiments (invalid or non-standard GPT sentiments)
missed_sentiments_count = len(balanced_data) - len(filtered_data)

# Count the cleaned and filtered sentiment values
gpt_sentiment_counts = filtered_data['gpt_sentiment_cleaned'].value_counts()
sentiment_counts = balanced_data['sentiment'].value_counts()

# Calculate the total number of valid GPT sentiments and original sentiments
total_gpt_sentiments = gpt_sentiment_counts.sum()
total_sentiments = sentiment_counts.sum()

# Print the sentiment counts and totals
print("Filtered GPT Sentiment Counts (positive/negative only):")
print(gpt_sentiment_counts)

print("\nOriginal Sentiment Counts:")
print(sentiment_counts)

print(f"\nTotal valid GPT sentiments analyzed: {total_gpt_sentiments}")
...
y_pred = filtered_data['gpt_sentiment_cleaned']  # GPT-generated sentiments
...
print(filtered_data[filtered_data['misclassified']][['sentiment', 'gpt_sentiment_cleaned']].head(10))
=======
from dataprep import *
from gpt import *
import pandas as pd
